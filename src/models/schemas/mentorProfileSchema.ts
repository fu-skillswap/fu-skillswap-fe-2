...
export const mentorAchievementSchema = yup.object().shape({
  ...
  awardDescription: yup
    .string()
    .trim()
    .required('Vui lòng nhập mô tả giải thưởng hoặc thành tích.'),
...
export const mentorProfileSchema = yup.object().shape({
  ...
  subjectResults: yup.array().of(mentorSubjectSchema).optional().default([]),
  projects: yup.array().of(mentorProjectSchema).optional().default([]),
  achievements: yup.array().of(mentorAchievementSchema).optional().default([]),
  agreeTerms: yup
    .boolean()
