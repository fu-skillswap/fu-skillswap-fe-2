...
// All requests that receive a 401 share this promise, preventing parallel token refreshes.
...
async function refreshAccessToken() {
  if (!refreshPromise)
    refreshPromise = requestEnvelope<TokenResponse>('/api/auth/refresh', { method: 'POST' }, true)
      .then(({ accessToken }) => {
