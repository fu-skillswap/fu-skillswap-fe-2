...
export function StudentOnboardingView({ locale }: { locale: string }) {
  ...
  useEffect(() => {
    if (user?.fullName)
      setForm((current) =>
        current.displayName ? current : { ...current, displayName: user.fullName },
      );
  }, [user?.fullName]);
  ...
  const selectProgram = async (programId: string) => {
    ...
    if (!programId) return;
    try {
      setSpecializations(await studentProfileService.getSpecializations(programId));
    } catch (reason) {
      setError(
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setError(undefined);
    setFieldErrors({});
    const missing = ['studentCode', 'campusId', 'programId', 'specializationId'].filter(
      (key) => !form[key as keyof FormValues],
    );
    if (missing.length) {
                ...
                <>
                  <label>
                    ...
                    <input
                      ...
                      required
                    />
                    {fieldErrors.studentCode && <small>{fieldErrors.studentCode}</small>}
                  </label>
                  <label>
                    Tên hiển thị
                    <input
                  ...
                  <div className="figma-profile-grid">
                    <label>
                      ...
                      {fieldErrors.campusId && <small>{fieldErrors.campusId}</small>}
                    </label>
                    <label>
                      Ngành học
                      <select
                      ...
                      {fieldErrors.programId && <small>{fieldErrors.programId}</small>}
                    </label>
                  </div>
                  <label>
                    Chuyên ngành
                    <select
                      ...
                      onChange={(event) => update('specializationId', event.target.value)}
                      disabled={!form.programId}
                      required
                    >
                      <option value="">
                        {form.programId ? 'Chọn chuyên ngành' : 'Chọn ngành trước'}
                      </option>
                    ...
                    {fieldErrors.specializationId && <small>{fieldErrors.specializationId}</small>}
                  </label>
                  <div className="figma-profile-grid">
                    <label>
                      Học kỳ
                    ...
                    <label>
                      ...
                      <input
                        ...
                        onChange={(event) => update('intakeYear', event.target.value)}
                        required
                      />
                    </label>
                  </div>
                  <label className="figma-profile-check">
                    <input
                      ...
                      onChange={(event) => update('isAlumni', event.target.checked)}
                    />{' '}
                    Tôi đã tốt nghiệp
                  </label>
                  {form.isAlumni && (
