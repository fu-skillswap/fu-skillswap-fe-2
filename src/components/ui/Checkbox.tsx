...
export function Checkbox({ label, id, name, className = '', disabled, ...props }: CheckboxProps) {
  ...
  return (
    ...
    >
      ...
      />
      <span
        className="w-4.5 h-4.5 rounded border border-solid border-border-strong bg-white flex items-center justify-center transition-all duration-150 peer-checked:bg-primary peer-checked:border-primary peer-checked:[&>svg]:opacity-100 peer-focus-visible:ring-2 peer-focus-visible:ring-primary-border"
        aria-hidden="true"
      >
        <svg
          className="w-3 h-3 text-white opacity-0 transition-opacity duration-100"
          viewBox="0 0 14 14"
          fill="none"
          stroke="currentColor"
        >
